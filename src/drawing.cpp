...
double distance_between_points(Point p1, Point p2) noexcept {
    return internal::pythagorean_theoreme(
        internal::difference(p1.x, p2.x), internal::difference(p1.y, p2.y));
}

namespace internal {
int difference(int a, int b) noexcept {
    return std::abs(a - b);
}
...
Angle_rad Angle_rad::pi() noexcept {
    return Angle_rad(3.14159265359);
}

Angle_rad::Angle_rad(double value) noexcept : value(value) {}
...
// Factor the sin/cos calculations away
